...
class Securepay
{
    ...
    function go(Plan $plan, Payment $payment)
    {
        ...
        $string = $user->email . '|' . $user->name . '|' . $buyer_phone . '|' . $verify_url . '|' . $payment->payment_code . '|' . $payment->remarks . '|' . $verify_url . '|' . $payment->amount . '|' . $this->uid;

        $sign = hash_hmac('sha256', $string, $this->checksum_token);

        $post_data = 'buyer_name=' . urlencode($user->name) . '&token=' . urlencode($this->auth_token) . '&callback_url=' . urlencode($verify_url) . '&redirect_url=' . urlencode($verify_url) . '&order_number=' . urlencode($payment->payment_code) . '&buyer_email=' . urlencode($user->email) . '&buyer_phone=' . urlencode($buyer_phone) . '&transaction_amount=' . urlencode($payment->amount) . '&product_description=' . urlencode($payment->remarks) . '&redirect_post=' . urlencode('true') . '&checksum=' . urlencode($sign);
